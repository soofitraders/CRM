...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.log(`Utilization export - Format: ${format}, User: ${user.email}`)
    ...
    try {
      if (format === 'csv') {
        ...
        fileBuffer = await exportToPDF('Utilization Report', exportData as any, columns as any)
    ...
    logger.error('Utilization export error:', error)
    return NextResponse.json(
