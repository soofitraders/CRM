...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Export request - Format: ${format}, User: ${user.email}, Module: BOOKINGS`)
    ...
    console.log('Fetching bookings with filter:', JSON.stringify(filter))
    ...
    console.log(`Fetched ${bookings.length} bookings for export`)
    ...
    try {
      if (format === 'csv') {
        ...
        console.log(`PDF generated: ${fileBuffer.length} bytes`)
    ...
    console.log(`Returning file: ${filename} (${fileBuffer.length} bytes)`)
