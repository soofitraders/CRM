...
export async function GET(
  ...
  try {
    console.log('[API] Booking GET request for ID:', params.id)
    const session = await getServerSession(authOptions)
    if (!session?.user) {
      console.log('[API] Booking: Unauthorized - no session')
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    console.log('[API] Booking: Connecting to database...')
    await connectDB()
    console.log('[API] Booking: Database connected')

    let booking
    try {
      console.log('[API] Booking: Querying database...')
      ...
      console.log('[API] Booking: Query completed, booking found:', !!booking)
    } catch (queryError: any) {
      console.error('[API] Booking: Database query error:', queryError)
      console.error('[API] Booking: Error name:', queryError.name)
      console.error('[API] Booking: Error message:', queryError.message)
      console.error('[API] Booking: Error stack:', queryError.stack)
      
      // Try to get booking without populate as fallback
      try {
        console.log('[API] Booking: Attempting fallback query without populate')
        booking = await Booking.findById(params.id).lean().exec()
        console.log('[API] Booking: Fallback query successful')
      } catch (fallbackError: any) {
        console.error('[API] Booking: Fallback query also failed:', fallbackError)
    ...
    if (!booking) {
      console.log('[API] Booking: Booking not found for ID:', params.id)
      return NextResponse.json({ error: 'Booking not found' }, { status: 404 })
    }

    console.log('[API] Booking: Successfully fetched booking:', booking._id)
    return NextResponse.json({ booking })
  } catch (error: any) {
    console.error('[API] Booking: Top-level error:', error)
    console.error('[API] Booking: Error name:', error.name)
